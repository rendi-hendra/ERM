    ...
    <form method="post" action="<?= isset($kunjungan) ? base_url('kunjungan/update/' . $kunjungan['id']) : base_url('kunjungan/create') ?>" class="space-y-5">
      ...
      <div>
        ...
        <textarea type="text" name="keluhan" id="keluhan"
          ...
          placeholder="Contoh: Demam" require><?= esc($oldInput['keluhan'] ?? ($kunjungan['keluhan']) ?? '') ?></textarea>
      ...
      <div class="grid grid-cols-1 sm:grid-cols-2 gap-5">
        ...
        <div>
          ...
          <p class="text-xs text-gray-500 mt-2">
            Asuransi hanya diperlukan jika metode pembayaran asuransi
          </p>
        </div>
      </div>

      <div class="pt-3">
        <a href="<?= base_url('kunjungan') ?>" class="mr-3 px-4 py-2 rounded-lg bg-gray-200 text-gray-800 hover:bg-gray-300 transition">
          Kembali
        </a>
        <button type="submit"
